...
cd /gitian/gitian-builder
 ...
 #test -f  /data/gitian-builder/base-trusty-i386  || ./bin/make-base-vm --lxc --arch i386  --suite trusty 
 #test -f  /data/gitian-builder/base-trusty-i386  || cp -av /gitian/gitian-builder/base-trusty-i386  /data/gitian-builder
 test -f  /data/gitian-builder/base-trusty-amd64 || ./bin/make-base-vm --lxc --arch amd64 --suite trusty 
 test -f  /data/gitian-builder/base-trusty-amd64 || cp -av /gitian/gitian-builder/base-trusty-amd64  /data/gitian-builder
echo "Now you're ready to run a build"
echo "docker run -it --rm --privileged -c $(pwd)/${GD_BUILD_COIN}:/gitian/${GD_BUILD_COIN} -v $(pwd)/gitian-builder:/gitian/gitian-builder  guruvan/gitian-builder"
